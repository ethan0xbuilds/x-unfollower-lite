...
const XUL_I18N_FALLBACKS = {
  en: {
    ...
    logLocating: "Scrolling to find @$1$…",
  },
  zh: {
    badgeQueued: "待取关",
    badgeActive: "取关中…",
...
function xulT(key, substitutions) {
  try {
    // chrome.i18n also throws "Extension context invalidated" after reload
    if (typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.id) {
      const msg = chrome.i18n.getMessage(key, substitutions);
      if (msg) return msg;
    }
  } catch {
    /* context dead or missing message */
  }
  const pack = XUL_I18N_FALLBACKS[xulUiLang()] || XUL_I18N_FALLBACKS.en;
  let fb = pack[key] || XUL_I18N_FALLBACKS.en[key];
  if (fb && substitutions != null) {
    const arr = Array.isArray(substitutions) ? substitutions : [substitutions];
    arr.forEach((v, i) => {
      fb = fb.replace(new RegExp(`\\$${i + 1}`, "g"), String(v));
    });
    // Named-style leftovers used in some fallback strings
    if (arr[0] != null) {
      fb = fb.replace(/\$HANDLE\$/g, String(arr[0]));
    }
  }
  if (fb) return fb;
  return key;
